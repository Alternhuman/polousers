...
extern "C" void request_for(const char* service, marco_node* nodes){
	...
	socklen_t size_addr = sizeof(bind_addr);
	 ...
	 JSONObject j;
	 j[L"Command"] = new JSONValue(L"Marco");
     //j[L"max_nodes"] = new JSONValue(L"30");
    ...
    nodes = (marco_node*)malloc(return_hosts.size() * sizeof(marco_node));
    for (int i = 0; i < return_hosts.size(); ++i)
    {
        strcpy(nodes[i].address, return_hosts[i].c_str()/*, */);
    }
}