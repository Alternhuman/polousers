/*!
   \file pam_mkdpolohomedir.c
*/

/* PAM Polo Users module

	This module will create a users home directory if it does not exist
	in each node of the net when the session begins. This allows users
	to be present in the central database (such as nis, kerb or ldap) 
	without using a distributed file system or pre-creating a large number
	of directories and avoiding logging in each node in order to have the 
	home directory created.
   
   Here is a sample /etc/pam.d/login file for Debian GNU/Linux TODO
   2.1:
...
/*!
  Creates the home directory using MarcoPolo
